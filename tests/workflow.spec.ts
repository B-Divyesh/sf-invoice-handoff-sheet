...
test('a freelancer can create a handoff and add delivery and follow-up records', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Harbor launch notes' })).toBeVisible();
  ...
  await page.getByLabel('What you sent or asked').fill('Asked for payment timing.');
