...
export const storageKey = (demo: boolean) => `${demo ? 'demo:' : ''}${base}sheets`;
...
export function loadSheets(demo: boolean): Sheet[] {
  ...
  try { return JSON.parse(localStorage.getItem(storageKey(demo)) || '[]') as Sheet[]; }
  catch { return []; }
}

export function saveSheets(demo: boolean, sheets: Sheet[]) { localStorage.setItem(storageKey(demo), JSON.stringify(sheets)); }
export function resetDemo() { localStorage.removeItem(storageKey(true)); }
export function csvEscape(value: string) { return `"${value.replaceAll('"', '""')}"`; }
export function followUpCsv(sheet: Sheet) {
  const header = ['Date', 'Method', 'Note', 'Outcome', 'Invoice', 'Client'];
  const rows = sheet.followUps.map((f) => [f.date, f.method, f.note, f.outcome, sheet.invoiceId, sheet.client]);
  return [header, ...rows].map((row) => row.map(csvEscape).join(',')).join('\n');
}

function htmlEscape(value = '') { return value.replace(/[&<>'"]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' }[c]!)); }
export function handoffHtml(sheet: Sheet) {
  const milestones = sheet.milestones.map((m) => `<li><strong>${htmlEscape(m.title)}</strong><br>Delivered: ${htmlEscape(m.deliveredOn || 'Not set')}${m.evidence ? `<br>Proof: <a href="${htmlEscape(m.evidence)}">${htmlEscape(m.evidence)}</a>` : ''}${m.acceptedBy ? `<br>Accepted by: ${htmlEscape(m.acceptedBy)}${m.acceptedOn ? ` on ${htmlEscape(m.acceptedOn)}` : ''}` : ''}</li>`).join('') || '<li>No delivery milestones recorded.</li>';
  const followUps = sheet.followUps.map((f) => `<li><strong>${htmlEscape(f.date)} · ${htmlEscape(f.method)}</strong><br>${htmlEscape(f.note)}<br>Outcome: ${htmlEscape(f.outcome)}</li>`).join('') || '<li>No follow-ups recorded.</li>';
